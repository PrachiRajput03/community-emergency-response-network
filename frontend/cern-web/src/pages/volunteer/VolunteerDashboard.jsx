...
export default function VolunteerDashboard() {
  ...
  const loadData = () => {
    ...
    Promise.all([
      ...
      .catch((err) => setError(err.response?.data?.message || 'Failed to load emergency data.'))
  ...
  const activeAssigned = assigned.filter((e) => e.status === EMERGENCY_STATUS.IN_PROGRESS)
  const resolvedCount = assigned.filter((e) => e.status === EMERGENCY_STATUS.RESOLVED).length
    ...
    <DashboardLayout title="Dashboard">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="font-display text-xl font-bold text-ink">{user?.name || 'Volunteer'}</p>
        </div>
        <div className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-brand-green/15 border border-brand-green/30 text-brand-green text-xs font-semibold">
          <span className="w-1.5 h-1.5 rounded-full bg-brand-green animate-pulse" /> ONLINE
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-8">
        <StatCard icon="🚨" label="Open Requests" value={loading ? '—' : openEmergencies.length} color="text-brand-amber" />
        <StatCard icon="🦺" label="Active Missions" value={loading ? '—' : activeAssigned.length} color="text-brand-blue" />
        <StatCard icon="✅" label="Resolved" value={loading ? '—' : resolvedCount} color="text-brand-green" />
      </div>
      ...
      {activeAssigned.length > 0 && (
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/volunteer/assigned" className="text-xs font-medium text-brand-red2 hover:underline">
              View All
            </Link>
          </div>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-ink3">{openEmergencies.length} nearby</span>
      </div>
      ...
      {loading ? (
        ...
        <div className="card p-10 text-center">
          ...
          <p className="text-sm text-ink2">No open emergencies right now. Stand by.</p>
        </div>
      ) : (
        <div className="space-y-3">
